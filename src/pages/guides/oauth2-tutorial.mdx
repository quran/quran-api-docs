...
- You can explore the list of all available APIs [here](/qdc-auth-v1).
- For example, to add a collection to your profile, you can use our interactive [collection API](/qdc-auth-v1/paths/v1-collections/post)
...
- Now, to make sure it was added, you can make a call to [get all collections API](/qdc-auth-v1/paths/v1-collections/get):
  - Put the `access_token` value in `o_t_h` header.
...
- `scope` value will come from `.env` file. You can read more about all the available scopes [here](/guides/scopes).
